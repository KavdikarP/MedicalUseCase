...
export FRONTEND_IMAGE_NAME=gcr.io/$PROJECT_ID/cymbal-insurance-frontend-$TIMESTAMP
...
kubectl apply -f frontend-deployment-cymbal.yaml
kubectl apply -f frontend-service-cymbal.yaml

# Update Kubernetes deployment with the new frontend image
kubectl set image deployment/cymbal-insurance-frontend frontend-container=${FRONTEND_IMAGE_NAME}

# Post-deployment checks for the frontend
echo "Verify the status of the frontend deployment:"
kubectl get deploy cymbal-insurance-frontend
echo "View the logs from the running frontend deployment:"
kubectl logs -l app=cymbal-insurance-frontend --tail=20
